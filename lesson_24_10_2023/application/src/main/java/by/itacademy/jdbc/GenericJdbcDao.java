...
public abstract class GenericJdbcDao<T> implements Dao<T> {
    ...
    private Connection createConnection() throws JdbcDaoException {
        try {
            return DriverManager.getConnection(properties.getDB_URL_PARAMETERS(), properties.getDB_USER(), properties.getDB_PASSWORD());
    ...
    private static void registerDriver(final String driverName) throws JdbcDaoException {
        try {
            ...
            if (isNotRegistered) {
                final org.postgresql.Driver driver = (Driver) driverClass.getDeclaredConstructor().newInstance();
